...
Usage:
    build-freebsd-pkg.py --version 0.3.0 --stage-root ./stage-root \
        --output dist/md_timesheet-0.3.0-freebsd-amd64.pkg

`--stage-root` must contain the package's files laid out at their final
absolute install paths (e.g. `<stage-root>/usr/local/bin/md_timesheet`). Every
...
to `{"origin", "version"}` (written by `fetch-freebsd-gtk.py`); it becomes the
manifest's `deps`, so the `.pkg` records its GTK runtime dependencies.
"""
...
def collect_files(stage_root: str) -> dict:
    ...
    package - md_timesheet installs nothing that needs a different owner.
    """
    files = {}
...
def parse_args(argv: list) -> argparse.Namespace:
    """Parse command-line arguments, applying md_timesheet defaults."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--name", default="md_timesheet")
    parser.add_argument("--origin", default="deskutils/md_timesheet")
    parser.add_argument("--version", required=True, help="e.g. 0.3.0 (no leading v)")
    parser.add_argument("--stage-root", required=True)
    parser.add_argument("--output", required=True)
    parser.add_argument("--deps-file", help="JSON {name: {origin, version}} for deps")
    parser.add_argument(
        "--comment", default="Keep your hours in a Markdown timesheet"
    )
    parser.add_argument("--maintainer", default="theholm@github.com")
    parser.add_argument("--www", default="https://github.com/TheHolm/md_timesheet")
    parser.add_argument("--license", default="AGPL-3.0-or-later")
    parser.add_argument("--category", default="deskutils")
    parser.add_argument(
        "--desc",
        default=(
            "md_timesheet is a small GTK4/libadwaita program that keeps a "
            "timesheet in a Markdown file, recording start and end times with "
            "rounded durations."
        ),
    )
